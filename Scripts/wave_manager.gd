...
@export var nome_das_ondas : Array[Onda] = []
@export var catalogo_de_upgrades : Array[Upgrade] = []
...
func iniciar_proxima_onda():
	...
	var onda_atual = nome_das_ondas.get(onda_atual_index) #dicionario de inimigos {"normal": 5, ...}
	texto_onda_atual.text = "Onda %s" % (onda_atual_index + 1)
	
	print("Iniciando ", texto_onda_atual.text)
	
	for i in range(onda_atual.normal):
		lista_de_inimigos.append("normal")
	for i in range(onda_atual.tanque):
		lista_de_inimigos.append("tanque")
	for i in range(onda_atual.rapido):
